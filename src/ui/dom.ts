...
/**
 * Section repliable du panneau lateral. L'etat plie/deplie est memorise
 * par cle, ce qui permet d'adapter l'espace sur un petit ecran.
 */
export function panelSection(opts: {
  title: string
  body: HTMLElement
  key: string
  actions?: Node[]
  ...
  const storageKey = `pixelforge.panel.${opts.key}`
  let collapsed = opts.collapsedByDefault ?? false
  try {
    const stored = localStorage.getItem(storageKey)
    if (stored !== null) collapsed = stored === '1'
  } catch { /* stockage indisponible : on garde la valeur par defaut */ }

  const chevron = el('button', {
    class: 'panel-toggle',
    'aria-label': 'Plier ou deplier',
    html: '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="m8 5 7 7-7 7"/></svg>',
  })

  const head = el('div', { class: 'panel-head' }, chevron, el('span', null, opts.title), el('span', { class: 'spacer' }))
  for (const action of opts.actions ?? []) head.appendChild(action)

  const section = el('section', {
    class: `panel ${opts.grow ? 'grow' : ''} ${opts.className ?? ''}`.trim(),
  }, head, opts.body)
